 ...
 * POST		"/{productId}"          		用戶(已登入)加入所關注商品 範例: "/favorites/1" 或 "/favorites/2"
 * DELETE   "/{productId}"          		用戶(已登入)移除所關注商品 範例: "/favorites/1" 或 "/favorites/2"
 * 
 * */
@RestController
...
public class FavoriteController {
	...
	// 獲取用戶(已登入)關注清單
	@GetMapping(value = {"", "/"})
	public ApiResponse<List<FavoriteProductDTO>> getFavoriteProduct(HttpSession httpSession) {
		// 是否有登入資料
